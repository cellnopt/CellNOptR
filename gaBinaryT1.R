...
#Function that produces the score for a specific bitstring
	...
	#cut the model according to bitstring	
		ModelCut<-Model
		ModelCut$interMat<-as.matrix(ModelCut$interMat[,as.logical(bitString)])
		ModelCut$notMat<-as.matrix(ModelCut$notMat[,as.logical(bitString)])
		ModelCut$reacID<-as.vector(ModelCut$reacID[as.logical(bitString)])
		SimListCut<-SimList
		SimListCut$finalCube<-as.matrix(SimListCut$finalCube[as.logical(bitString),])
		SimListCut$ixNeg<-as.matrix(SimListCut$ixNeg[as.logical(bitString),])
		SimListCut$ignoreCube<-as.matrix(SimListCut$ignoreCube[as.logical(bitString),])
		SimListCut$maxIx<-as.vector(SimListCut$maxIx[as.logical(bitString)])
