		...
		int* isState,int* nInputs,int *nBits,int nRows,int nCols)
{
	...
	for (i = 0; i < nRows; i++)
	{
		...
		miniTermPositions[i]=malloc(counter1*sizeof(int));
		counter1=0;
		for (j = 0; j < nCols; j++)
		{
			if(interMat[i][j]==1)
	...
	for (j = 0; j < nRows; j++)
	{
		...
		{
				counter1=0;
				miniTermInputs[j]=malloc(nInputs[j]*sizeof(int));
				for (i = 0; i < nRows; i++)
				{
					if(adjMat[i][j]==1)
	...
	miniTerms=malloc(nRows*sizeof(int**));
	for (i = 0; i < nRows; i++)
	{
		miniTerms[i]=malloc(nInputs[i]*sizeof(int*));

		for (j = 0; j < nInputs[i]; j++)
		{
			miniTerms[i][j]=malloc(nMiniTerms[i]*sizeof(int));
	...
	//Not that we have the miniterms it is easier to parse to
	//Truth tables
	for (i = 0; i < nRows; i++)
	{
		truthTables[i]=malloc(nBits[i]*sizeof(int));
