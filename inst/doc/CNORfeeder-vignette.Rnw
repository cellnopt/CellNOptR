...
biocLite("CellNOptR")
@
The following code installs the \emph{CNORfeeder} dependencies that are available on CRAN website. Open a R session and type:
...
Different methods can be used to reverse-engineer a network purely from data:
\begin{enumerate}
...
The inferred DDN is integrated with the prior knowledge, taking into account that networks inferred directly from data (DDNs) are limited only to measured, inhibited and stimulated nodes, thus a link in the DDN can correspond to multiple links in th PKN.
...
modelIntegrWeight <- weighting(modelIntegr=modelIntegr, PKNmodel=DreamModel,
                               CNOlist=CNOlistDREAM, integrFac=10, PPI=FALSE)
@
...
\begin{center}
\begin{tabular}{cc}
    {\includegraphics[width=7cm]{optModel}} 
    &
    {\includegraphics[width=8cm]{SimResultsT1_1}} \\
\end{tabular}
