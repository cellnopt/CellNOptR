...
knowledge networks and perturbation data with \emph{CellNOptR} \\\small{(version 1.3.28
and above)}}




\author{Camille Terfve, Thomas Cokelaer, Julio Saez-Rodriguez}
...
file CellNOptR-examples.R which contains some  R commands being used in this
tutorial.  We recommend that you use that script as a template for your
...
# deprecated since version 1.3.28: no need to use indexFinder anymore
...
following 3 step in 1 command line. However, the description of the 3 steps remains
...
<<expand, eval=TRUE>>=
ToyNCNOcutComp <- preprocessing(CNOlistToy, ToyModel, expansion=TRUE,
...
residual error. Then, we create an initial bit string for the optimisation,
...
cutAndPlot(model=ToyNCNOcutCompExp, bStrings=list(ToyT1opt$bString),
    CNOlist=CNOlistToy,plotPDF=FALSE)
@
\end{center}
\begin{center}
%TOSWITCH
...
namesFilesToy<-list(
    ...
    evolFitT2=NA,
    simResultsT1="ToyNCNOcutCompExpSimResultsT1.pdf",
    simResultsT2=NA,
...
Having obtained a pre-processed model, we compute the residual error (\emph{residualError}), prepare the model for simulation (\emph{prep4sim}), and run the training function (\emph{gaBinaryT1}). Finally,we plot the results of our training (\emph{cutAndPlot}, see figure \ref{fig:DREAMres}) as well as the information about the evolution of fit during optimisation (\emph{plotFit}).
...
namesFilesToy<-list(
    ...
    simResultsT2="ToyNCNOcutCompExp2SimResultsT1T2.pdf",
    simResultsT1="ToyNCNOcutCompExp2SimResultsT1.pdf",
    scaffold="Scaffold.sif",
    scaffoldDot="Scaffold.dot",
    tscaffold="TimesScaffold.EA",
    wscaffold="weightsScaffold.EA",
