...
knowledge networks and perturbation data with \emph{CellNOptR} \\\small{(version 1.3.27
and above)}}
...
<<expand, eval=TRUE>>=
results <- preprocessing(CNOlistToy, ToyModel, expansion=TRUE, 
...
results = preprocessing(CNOlistToy2,ToyModel2, verbose=FALSE)
indicesToyNCNOcutComp2<-results$indices
ToyNCNOcutCompExp2<-results$model
