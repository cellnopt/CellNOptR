...
CNORdt is an add-on to CellNOptR, a software package that trains logic models to data. More details and the package itself can be found using the following command to install:
...
The model and data are then loaded. These are taken from MacNamara et al.~REF and consist of a biologically realistic toy model based on the EGFR signaling pathway and \emph{in silico}-generated data:
...
%initBstring=initBstring, verbose=TRUE, boolUpdates=c(10,20), divTime=10, MaxTime=100, lowerB=0.8, upperB=10)
%@
...
%SimList=fields4Sim, indexList=indexNONCcutComp,
%bStringT1=opt1$bString, SimResT1=simT1, verbose=TRUE, boolUpdates=c(10,20), divTime=10, lowerB=0.8, upperB=10)
%@
