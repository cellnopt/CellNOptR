...
indexOrig <- indexFinder(CNOlist=CNOlistPB, model=modelPB, verbose=T)

# find the indexes of the non-observables and the non-controllable species
indexNONC <- findNONC(model=modelPB, indexes=indexOrig, verbose=T)

# cut the data according to 'indexNONC'
modelCut <- cutNONC(model=modelPB, NONCindexes=indexNONC)

# find the indexes again as model may have changed
indexNONCcut <- indexFinder(CNOlist=CNOlistPB, model=modelCut)

# compress the model
modelCutCompress <- compressModel(model=modelCut, indexes=indexNONCcut)

# find the indexes again
indexNONCcutComp <- indexFinder(CNOlist=CNOlistPB, model=modelCutCompress)
...
<<getData, eval=FALSE>>=
cutAndPlotResultsTimeScale(model=modelCutCompressExpand, bString=opt1$bString,
simList=fields4Sim, CNOlist=CNOlistPB, indexList=indexNONCcutComp, boolUpdates=c(10,20),
...
\caption{The fit of the model over the early time phase ($t=0$ to $t=10$). The black triangles are the experimental measurements across the time course. The simulation is represented by a blue dashed line. The color code represents the normalized error between the simulation and the data. The colors range from white (perfect fit) to red (poor fit) spanning heat map coloration in between.
}
\label{fig:halfPlot}
\end{center}
\end{figure}
...
simT1 = simulatorTimeScale(CNOlist=CNOlistPB,
...
CNOlist=CNOlistPB, indexList=indexNONCcutComp, boolUpdates=c(10,20),
divTime=10, lowerB=0.8, upperB=10)
@
