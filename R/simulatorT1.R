...
simulatorT1<-function(CNOlist,Model,SimList,indexList){
    ...
    # simulator at t1. 
    # WARNING: simulatorT0 calls this function. There is a tricky part in
    # simualtorT1 to handle the NA case . Be very careful when changinh this
    # function. TC. March 2012
    ...
    while(termCheck1 && termCheck2){
        ...
        # (column in finalCube) and a set of rows for each reac (where a 
        # set contains as many rows as conditions) all concatenated into one
        # long column.
        ...
        if(nReacs > 1){
            ...
            tempStore<-apply(SimList$finalCube,2,
                function(x){return(outputPrev[,x])})
        ...
        # Set to NA the values that are "dummies", so they won't influence the
        # min.
        ...
        # Compute all the ands by taking, for each gate, the min value across the
        # inputs of that gate.
        if(nReacs > 1){
            outputCube<-apply(tempStore,1,minNA)

        # OutputCube is now a vector of length (nCond*nReacs) that contains the
        # input of each reaction in each condition, concatenated as such
        # allcond4reac1,allcond4reac2,etc...  This is transformed into a matrix
        # with a column for each reac and a row for each cond.
            outputCube<-matrix(outputCube, nrow=nCond,ncol=nReacs)
            
        # Go through each species, and if it has inputs, then take the max
        # across those input reactions i.e. compute the ORs.
        ...
        # Replace NAs with zeros to avoid having the NA    penalty applying to
        # unconnected species.
