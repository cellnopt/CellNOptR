normaliseCNOlist <-
function(CNOlist,EC50Data=0.5,HillCoef=2,EC50Noise=0.1,Detection=0,Saturation=Inf,ChangeTh=0,Norm2TorCtrl="time"){
#check that CNOlist is a CNOlist
	if(!is.list(CNOlist)) stop("This function expects as input a CNOlist as output by makeCNOlist")
	if(all(names(CNOlist) != c("namesCues","namesStimuli","namesInhibitors","namesSignals","timeSignals","valueCues","valueInhibitors","valueStimuli","valueSignals"))){
		stop("This function expects as input a CNOlist as output by makeCNOlist")
		}
#Check that the parameters have the right format
	if(class(EC50Data) != "numeric" | length(EC50Data) != 1) warning("The parameter 'EC50Data' should be a single numeric value")
	if(class(EC50Noise) != "numeric" | length(EC50Noise) != 1) warning("The parameter 'EC50Noise' should be a single numeric value")
	if((class(HillCoef) != "numeric" & class(HillCoef) != "integer") | length(HillCoef) != 1) warning("The parameter 'HillCoef' should be a single numeric value")
	if((class(Detection) != "numeric" & class(Detection) != "integer") | length(Detection) != 1) warning("The parameter 'Detection' should be a single numeric value")
	if((class(ChangeTh) != "numeric" & class(ChangeTh) != "integer") | length(ChangeTh) != 1) warning("The parameter 'ChangeTh' should be a single numeric value")
	if((class(Saturation) != "numeric" & class(Saturation) != "integer") | length(Saturation) != 1) warning("The parameter 'Saturation' should be a single numeric value")
	if(length(Norm2TorCtrl) != 1 | length(match(Norm2TorCtrl, c("time","ctrl"))) != 1) warning("The parameter 'Norm2TorCtrl' should be either 'time' or 'ctrl'")
...
#(ie each measurement is compared to the exact same condition at time 0)
	if(Norm2TorCtrl == "time"){
		...
		FoldChangeList[[1]]<-matrix(0,ncol=dim(FoldChangeList[[1]])[2],nrow=dim(FoldChangeList[[1]])[1])	
		}else{
...
#First I treat the rows that are not ctrls				
					if(sum(CNOlist$valueStimuli[n,]) != min(apply(CNOlist$valueStimuli,MARGIN=1,sum))){
						ctrlRow<-intersect(which(apply(CNOlist$valueStimuli,MARGIN=1,sum) == min(apply(CNOlist$valueStimuli,MARGIN=1,sum))),which(apply(CNOlist$valueInhibitors,MARGIN=1,function(x) all(x == CNOlist$valueInhibitors[n,]))))
...
#for that particular signal (excluding values out of the dynamic range)
	...
	PenaltyList<-lapply(PenaltyList, function(x){x<-apply(x,MARGIN=1,function(y){y<-y/signalMax});return(t(x))})
...
#Now I make the data go through the Hill function
	...
	HillData<-lapply(HillData,function(x) {x^HillCoef/((EC50Data^HillCoef)+(x^HillCoef))} )
