  ...
  for (p in 1:length(CNOlist$namesSignals)){  
    ...
    for (i in 1:length(CNOlist$namesStimuli)){
      ...
      if (length(test1_index)==0 || length(na.omit(test1[-1]))==0){
        ...
        print(paste(CNOlist$namesSignals[p],": the condition with stimulus", CNOlist$namesStimuli[i] , "is missing or NA"))
        ...
        if (length(which(abs(check) > k*check_sd))>0){
          ...
          for (j in 1:length(CNOlist$namesInhibitors)){
            ...
            if (length(test2_index)==0 || length(na.omit(test2[-1]))==0){
              ...
              print(paste(CNOlist$namesSignals[p],": the condition with stimulus", CNOlist$namesStimuli[i] ,"and inhibitor", CNOlist$namesInhibitors[j]," is missing or NA"))
