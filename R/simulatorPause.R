simulatorPause <- function(CNOlist, Model, SimList, indexList, boolUpdates, delayThresh) {
	...
	delayThreshTot = rep(NA,nReacs*nCond)
	#strongWeakTot = rep(NA,length(nReacs*nCond))
	for(a in 1:nReacs) {
		delayThreshTot[(1:nCond)+(nCond*(a-1))] = rep(delayThresh[a], nCond)
	#	strongWeakTot[(1:9)+(9*(a-1))] = rep(strongWeak[a], nCond)
	}
