...
#check that all of the signals in CNOlist$namesSignals match to one species in Model$namesSpecies
	...
	if(any(signalsMatch == 0)){
		warning(paste("The following signals from your CNOlist do not match any of the species in your model, and should be removed:",toString(CNOlist$namesSignals[which(signalsMatch == 0)])))
		}
	}
