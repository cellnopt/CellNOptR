...
# Rgraphviz (v1.32).
    ...
    if (typeof(model) == "character"){
        ...
        IntegrLG<-rep(0,length(edges))
        IntegrG<-rep(0,length(edges))
        IntegrB<-rep(0,length(edges))
        IntegrR<-rep(0,length(edges))# 
    }
    ...
    # This block is the tricky part of the function. Change with care.
        ...
        optIntegrLG<-rep(0,length(optimBStimes))
        if (!is.na(indexIntegerLG[1])){
            optIntegrLG[indexIntegerLG]<-1
        }
        optIntegrG<-rep(0,length(optimBStimes))
        if (!is.na(indexIntegerG[1])){
            optIntegrG[indexIntegerG]<-1
        }
        optIntegrB<-rep(0,length(optimBStimes))
        if (!is.na(indexIntegerB[1])){
            optIntegrB[indexIntegerB]<-1
        }
        optIntegrR<-rep(0,length(optimBStimes))
        if (!is.na(indexIntegerR[1])){
            optIntegrR[indexIntegerR]<-1
        }
        ...
        for (i in 1:dim(reacs)[1]){
          ...
          if (length(inputs)==1){
            ...
            IntegrLG[CountReac]<-optIntegrLG[i]
            IntegrG[CountReac]<-optIntegrG[i]
            IntegrR[CountReac]<-optIntegrR[i]
            IntegrB[CountReac]<-optIntegrB[i]
    ...
    res = createEdgeAttrs(v1, v2, edges, BStimes, IntegrLG, IntegrG, IntegrB, IntegrR)
...
createEdgeAttrs <- function(v1, v2, edges, BStimes, IntegrLG, IntegrG, IntegrB, IntegrR){
...
#label[edgename] = as.character(v)
    ...
    indexI<-intersect(which(IntegrLG==1), which(BStimes==1))
    edgecolor[indexI]<-"green"
    indexI<-intersect(which(IntegrG==1), which(BStimes==1))
    edgecolor[indexI]<-"forestgreen"
    indexI<-intersect(which(IntegrB==1), which(BStimes==1))
    edgecolor[indexI]<-"blue"
    indexI<-intersect(which(IntegrR==1), which(BStimes==1))
