writeNetwork <-
function(ModelOriginal,ModelComprExpanded,optimResT1,optimResT2,CNOlist){
...
#the max of the score of the and or of that particular branch
	if(length(ModelComprExpanded$newANDs) != 0){
		for(i in 1:length(ModelComprExpanded$newANDs)){
			...
			BStimes[unlist(branches)]<-pmax(BStimes[match(unlist(branches),reacs)],BStimes[match(names(ModelComprExpanded$newANDs)[i],reacs)])
			}
		}		
...
adj<-cbind(adj[order(adj[,3],decreasing=TRUE),1],adj[order(adj[,3],decreasing=TRUE),2],adj[order(adj[,3],decreasing=TRUE),3])
adj<-unique(adj)
...
#absent from the model (0), present at t1(1) or present at t2(2)
	writeDot(dotNodes=dN,dotMatrix=dM,Model=ModelOriginal,fileName=paste(deparse(substitute(ModelOriginal)),"PKN.dot",sep=""))
	write.table(sifFile[,1:3],file=paste(deparse(substitute(ModelOriginal)),"PKN.sif",sep=""),row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
	write.table(EAweights,file=paste(deparse(substitute(ModelOriginal)),"TimesPKN.EA",sep=""),row.names=FALSE,col.names="Times",quote=FALSE,sep="\t")	
	write.table(nodesAttr,file=paste(deparse(substitute(ModelOriginal)),"nodesPKN.NA",sep=""),row.names=FALSE,col.names="NodesType",quote=FALSE,sep="\t")	
	}
