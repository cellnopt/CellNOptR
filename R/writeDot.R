...
#write the nodes
	for(i in 1:length(nodes)){
		...
		if(is.na(nClass[i])){
			cat(' [color="black" shape="ellipse" style="solid" label="',file=fileName,append=TRUE,sep="")
			cat(nodes[i],file=fileName,append=TRUE,sep="")
			cat('" fontname=Helvetica fontsize=22.0 ];\n',file=fileName,append=TRUE,sep="")
			}else{
				if(nClass[i] == "signal"){
					cat(' [color="lightblue" shape="ellipse" style="filled" label="',file=fileName,append=TRUE,sep="")
					}
				if(nClass[i] == "inhibited"){
					cat(' [color="orangered" shape="ellipse" style="filled" label="',file=fileName,append=TRUE,sep="")
					}
				if(nClass[i] == "stimulated"){
					cat(' [color="olivedrab3" shape="ellipse" style="filled" label="',file=fileName,append=TRUE,sep="")
					}
				if(nClass[i] == "ncno"){
					cat(' [color="lightblue" shape="ellipse" style="filled" label="',file=fileName,append=TRUE,sep="")
					}
				if(nClass[i] == "compressed"){
					cat(' [color="black" shape="ellipse" style="dashed" label="',file=fileName,append=TRUE,sep="")
					}
				if(nClass[i] == "dummy"){
...
#a. get the reaction input->output
		cat(paste(match(dotMatrix[i,1],nodes),"->",match(dotMatrix[i,3],nodes),sep=" "),file=fileName,append=TRUE,sep="")
#b.get the color
		if(dotMatrix[i,4] == 0){
			cat('[ color="grey90" label="" weight="1.000000" penwidth="',file=fileName,append=TRUE,sep="")
			}
		if(dotMatrix[i,4] == 1){
			cat('[ color="forestgreen" label="" weight="1.000000" penwidth="',file=fileName,append=TRUE,sep="")
			}	
		if(dotMatrix[i,4] == 2){
			cat('[ color="blue" label="" weight="1.000000" penwidth="',file=fileName,append=TRUE,sep="")
			}	
