...
plotCNOlistLarge <-
function(CNOlist,nsplit=4){
#check that CNOlist is a CNOlist
	if(!is.list(CNOlist)) stop("This function expects as input a CNOlist as output by makeCNOlist or normaliseCNOlist")
	...
	par(mfrow=c(nr=dim(CNOlist$valueSignals[[1]])[1]+1,nc=dim(CNOlist$valueSignals[[1]])[2]+2),cex=0.5,pch=20,mar=c(0.5,0.5,0,0),oma=c(3,2,2,2))
	...
	for(c in 1:dim(CNOlist$valueSignals[[1]])[2]){
		...
		text(labels=CNOlist$namesSignals[c],x=((xVal[length(xVal)]-xVal[1])/2),y=(yMin+((yMax-yMin)/2)),cex=1)
		}
		plot(x=xVal, y=rep(-5,length(xVal)), ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n")
		text(labels="Stim",x=((xVal[length(xVal)]-xVal[1])/2),y=(yMin+((yMax-yMin)/2)),cex=1)
		plot(x=xVal, y=rep(-5,length(xVal)), ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n")
		text(labels="Inh",x=((xVal[length(xVal)]-xVal[1])/2),y=(yMin+((yMax-yMin)/2)),cex=1)
	par(fg="black",mar=c(0.5,0.5,0,0))	
	for(r in 1:dim(CNOlist$valueSignals[[1]])[1]){
		for(c in 1:dim(CNOlist$valueSignals[[1]])[2]){
			yVal<-lapply(CNOlist$valueSignals,function(x) {x[r,c]})
			if(r == dim(CNOlist$valueSignals[[1]])[1] && c== 1){
			...
			if(c != 1 && r!=1 && r!=dim(CNOlist$valueSignals[[1]])[1] && c != dim(CNOlist$valueSignals[[1]])[1]+1){
				plot(x=xVal,y=yVal,ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n")
				lines(x=xVal,y=yVal,ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n")
				}
			}
		image(t(matrix(CNOlist$valueStimuli[r,],nrow=1)),col=c("white","black"),xaxt="n",yaxt="n")
		if(r == dim(CNOlist$valueSignals[[1]])[1]){
			axis(side=1,at=seq(from=0, to=1,length.out=length(CNOlist$namesStimuli)),labels=CNOlist$namesStimuli,las=3,cex.axis=1)
			}
		image(t(matrix(CNOlist$valueInhibitors[r,],nrow=1)),col=c("white","black"),xaxt="n",yaxt="n")	
		if(r == dim(CNOlist$valueSignals[[1]])[1]){
			axis(side=1,at=seq(from=0, to=1,length.out=length(CNOlist$namesInhibitors)),labels=paste(CNOlist$namesInhibitors,"-i",sep=""),las=3,cex.axis=1)
			}
		}
	}			
	}
