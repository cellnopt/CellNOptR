...
#Create the first one, that goes from input to the first output
					...
					if(Model$notMat[sp,outReac[1]] == 1) newNots[input]<-1
					newReacIDs<-paste(names(newReacs)[which(newReacs == -1)],"=",names(newReacs)[which(newReacs == 1)],sep="")
...
#now create the ones that go to the following outputs, if any
					if(length(outReac) > 1){
						for(r in 2:length(outReac)){
							newReacs<-cbind(newReacs, (Model$interMat[,inReac]+Model$interMat[,outReac[r]]))
							nNtemp<-Model$notMat[,inReac]
							if(Model$notMat[sp,outReac[r]] == 1) nNtemp[input]<-1
							newNots<-cbind(newNots,nNtemp)
							newReacIDstemp<-paste(rownames(newReacs)[which(newReacs[,r] == -1)],"=",rownames(newReacs)[which(newReacs[,r] == 1)],sep="")
							if(any(newNots[,r] == 1)) newReacIDstemp<-paste("!",newReacIDstemp,sep="")
...
#Find the input element(s)
						if(length(inReac) > 1) inputs<-apply(Model$interMat[,inReac],2,function(x) which(x == -1))
						if(length(inReac) == 1) inputs<-which(Model$interMat[,inReac] == -1)
...
#create the first reac, that goes from the first input to the output
							...
							if(Model$notMat[sp,outReac] == 1 && length(which(newReacs == -1)) == 1) newNots[inputs[1]]<-ifelse((newNots[inputs[1]] == 1), 0, 1)
...
#This makes the name of the new reac in cases where the input is an & gate								
									...
									newReacIDs<-paste(in1,"+",in2,"=",names(newReacs)[which(newReacs == 1)],sep="")
									indone<-2
									}
#Now create the reactions that go to from other inputs	to the output
							if(length(inReac) > 1){
								for(r in 2:length(inReac)){
									...
									if(Model$notMat[sp,outReac] == 1 && length(which(newReacs == -1)) == 1) nNtemp[inputs[indone+1]]<-ifelse((nNtemp[inputs[indone+1]] == 1), 0, 1)
									if(Model$notMat[sp,outReac] == 1 && length(which(newReacs == -1)) != 1) {
									...
									if(length(which(newReacs[,dim(newReacs)[2]] == -1)) == 1){
										...
										indone<-indone+1
											...
											newReacIDstemp<-paste(in1,"+",in2,"=",rownames(newReacs)[which(newReacs[,r] == 1)],sep="")
											newReacIDs<-c(newReacIDs,newReacIDstemp)
											indone<-indone+2
											}
									colnames(newReacs)<-newReacIDs
