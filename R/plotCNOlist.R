...
#check that CNOlist is a CNOlist
	...
	par(mfrow=c(nr=dim(CNOlist$valueSignals[[1]])[1]+1,nc=dim(CNOlist$valueSignals[[1]])[2]+1),cex=0.5,pch=20,mar=c(0.5,0.5,0,0),oma=c(3,2,2,2))
	...
	for(c in 1:dim(CNOlist$valueSignals[[1]])[2]){
		...
		text(labels=CNOlist$namesSignals[c],x=((xVal[length(xVal)]-xVal[1])/2),y=(yMin+((yMax-yMin)/2)),cex=2)
		}
		plot(x=xVal, y=rep(-5,length(xVal)), ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n")
	...
	for(r in 1:dim(CNOlist$valueSignals[[1]])[1]){
		...
		image(t(matrix(c(CNOlist$valueStimuli[r,],CNOlist$valueInhibitors[r,]),nrow=1)),col=c("white","black"),xaxt="n",yaxt="n")	
		if(r == dim(CNOlist$valueSignals[[1]])[1]){
			axis(side=1,at=seq(from=0, to=1,length.out=length(CNOlist$namesCues)),labels=c(CNOlist$namesStimuli,paste(CNOlist$namesInhibitors,"-i",sep="")),las=3,cex.axis=1)
			}
		}
