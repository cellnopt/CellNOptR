writeScaffold <-
function(ModelComprExpanded,optimResT1,optimResT2,ModelOriginal,CNOlist){
				...
				if(length(reacInput[[i]]) == 1){
					...
					nR<-nR+1
						...
						for(inp in 1:length(reacInput[[i]])){
							...
							sifFile[nR,2]<-ifelse(ModelComprExpanded$notMat[inp,i] == 1,-1,1)
							sifFile[nR,4]<-BStimes[i]
							sifFile[nR,5]<-weightsE[i]
							nR<-nR+1
							}
...
#This bit produces what I need for the dot file
	dM<-sifFile
	...
	#
	writeDot(dotNodes=dN,dotMatrix=dM,Model=ModelComprExpanded,fileName=paste(deparse(substitute(ModelModelComprExpanded)),"Scaffold.dot",sep=""))
	write.table(sifFile[,1:3],file=paste(deparse(substitute(ModelComprExpanded)),"Scaffold.sif",sep=""),row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
	write.table(EApresent,file=paste(deparse(substitute(ModelComprExpanded)),"TimesScaffold.EA",sep=""),row.names=FALSE,col.names="Times",quote=FALSE,sep="\t")
	write.table(EAweights,file=paste(deparse(substitute(ModelComprExpanded)),"weightsScaffold.EA",sep=""),row.names=FALSE,col.names="Weights",quote=FALSE,sep="\t")
	}
