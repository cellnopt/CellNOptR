...
#now extract the names of the signals
	...
	namesSignals<-sub(pattern="(DA:p-)",x=namesSignals, replacement="",perl=TRUE)	
	namesSignals<-sub(pattern="(DA:)",x=namesSignals, replacement="",perl=TRUE)
...
#3. The cues sub-data frame now contains 1 if the cue is present and NA otherwise,
				...
				for(n in timesRows[(whereTimes[1]+1):(whereTimes[1]+whereTimes[2])]){
					...
					for(n in timesRows[(whereTimes[1]+1):(whereTimes[1]+whereTimes[2])]){
						if(length(zerosCond[which(cues[n,zerosCond] == 1)]) == length(present)){
							if(all(zerosCond[which(cues[n,zerosCond] == 1)] == present) && length(which(cues[n,zerosCond] == 1)) != 0){
								valueSignals$t0[count,]<-as.numeric(dataset$dataMatrix[i,dataset$DVcol])
								newcues[count,]<-cues[n,]
								count=count+1
								}
...
#Build the valueInhibitors and valueStimuli  matrices, which are a subset of the cues one
	if(subfield == TRUE){
		valueInhibitors<-newcues[,grep(pattern="Inhibitor",x=colnames(cues),ignore.case=TRUE)]
		valueStimuli<-newcues[,-grep(pattern="Inhibitor",x=colnames(cues),ignore.case=TRUE)]
		}else{
			valueInhibitors<-newcues[,grep(pattern="(i$)",x=colnames(cues),ignore.case=FALSE,perl=TRUE)]
			valueStimuli<-newcues[,-grep(pattern="(i$)",x=colnames(cues),ignore.case=FALSE,perl=TRUE)]
			}
	...
	return(list(namesCues=namesCues,namesStimuli=namesStimuli,namesInhibitors=namesInhibitors,namesSignals=namesSignals,timeSignals=timeSignals,valueCues=newcues,valueInhibitors=valueInhibitors,valueStimuli=valueStimuli,valueSignals=valueSignals))
	}
