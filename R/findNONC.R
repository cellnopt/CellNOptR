findNONC <-
function(Model,indexes,verbose=FALSE){
...
#then this species is not observable
		if(all(is.infinite(distMatrix[sp,indexes$signals]))  && !any(indexes$signals == sp)) ncno<-c(ncno,sp)
		if(all(is.infinite(distMatrix[c(indexes$stimulated,indexes$inhibited),sp])) && !any(c(indexes$stimulated,indexes$inhibited) == sp)) ncno<-c(ncno,sp)
		}
	ncno<-ncno[-1]
	ncno<-unique(ncno)
#check that none of the cues or signals are in there
	...
	if(verbose) print(paste("The following species are not observable and/or not controllable:",toString(Model$namesSpecies[ncno])))
	return(ncno)
	}
