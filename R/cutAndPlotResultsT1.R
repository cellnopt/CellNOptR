cutAndPlotResultsT1 <-
function(Model,bString,SimList,CNOlist,indexList,plotPDF=FALSE){
	Modelcut<-Model
	...
	plotOptimResults(SimResults=SimResults,expResults=expResults,times=CNOlist$timeSignals[1:2],namesCues=CNOlist$namesCues,namesSignals=CNOlist$namesSignals,valueCues=CNOlist$valueCues)
	if(plotPDF == TRUE){
		plotOptimResultsPDF(SimResults=SimResults,expResults=expResults,times=CNOlist$timeSignals[1:2],fileName=paste(deparse(substitute(Model)),"SimResultsT1.pdf",sep=""),namesCues=CNOlist$namesCues,namesSignals=CNOlist$namesSignals,valueCues=CNOlist$valueCues)
		}
	}
