...
#Create the vector of names or reactions	
	createReacID<-function(x){
		...
		ID<-ifelse(neg, paste("!",x[1],"=",x[3],sep=""), paste(x[1],"=",x[3],sep=""))
		return(ID)
		}
...
#1.detect them
	AndNodes<-grep(pattern="(and\\d+$)",namesSpecies,perl=TRUE,ignore.case=FALSE)
	AndNodesV<-grep(pattern="(and\\d+$)",namesSpecies,perl=TRUE,ignore.case=FALSE,value=TRUE)
	if(length(AndNodes) != 0){
#2.go through them	
		for(i in 1:length(AndNodes)){
#3.store the name of the node, and find the associated reactions		
			node<-AndNodesV[i]
			AndsReacs<-c(grep(pattern=paste(node,"=",sep=""),reacID,ignore.case=FALSE),grep(pattern=paste(node,"$",sep=""),perl=TRUE,reacID,ignore.case=FALSE))
...
#5.create the new reacID
			...
			if(length(AndsReacs) == 3){
				output<-grep(pattern=paste(node,"=",sep=""),reacID,ignore.case=FALSE,value=TRUE)
				inputs<-grep(pattern=paste(node,"$",sep=""),perl=TRUE,reacID,ignore.case=FALSE,value=TRUE)
				output<-sub(pattern=node,x=output,replacement="",ignore.case=FALSE)
				inputs<-sub(pattern=paste("=",node,sep=""),x=inputs,replacement="+",ignore.case=FALSE)
				inputs<-paste(inputs,collapse='')
				newReacID<-paste(inputs,output,sep="")
				newReacID<-sub(pattern=paste("\\W{1}",output,sep=""),x=newReacID,replacement=output,ignore.case=FALSE,perl=TRUE)
				}else{
			#Second case: I have an 'and' with more than 2 inputs
					...
					if(length(output) == 1){
						...
						newReacID<-sub(pattern=paste("\\W{1}",output,sep=""),x=newReacID,replacement=output,ignore.case=FALSE,perl=TRUE)
							...
							output<-sub(pattern=paste(node,"=",sep=""),x=output,replacement="",ignore.case=FALSE)
							inputs<-grep(pattern=paste(node,"$",sep=""),perl=TRUE,reacID,ignore.case=FALSE,value=TRUE)
							inputs<-sub(pattern=paste("=",node,sep=""),x=inputs,replacement="+",ignore.case=FALSE)
							inputs<-paste(inputs,collapse='')
							inputs<-sub(pattern="\\W{1}$",x=inputs,replacement="",ignore.case=FALSE,perl=TRUE)
							newReacID<-paste(inputs,output,sep="=")
							}
					}
#5b.if there are more than one outputs, then the interMat and notMat created above are not correct
			output<-grep(pattern=paste(node,"=",sep=""),reacID,ignore.case=FALSE,value=FALSE)
			inputs<-grep(pattern=paste(node,"$",sep=""),perl=TRUE,reacID,ignore.case=FALSE,value=FALSE)
			if(length(output) != 1){
				newReac<-rowSums(interMat[,c(inputs,output[1])])
				newReacNot<-rowSums(notMat[,c(inputs,output[1])])
				for(i in 2:length(output)){
					newReac<-cbind(newReac,rowSums(interMat[,c(inputs,output[i])]))
					newReacNot<-cbind(newReacNot,rowSums(notMat[,c(inputs,output[i])]))
					}
				}
#6. remove the old reactions and replace them by the new ones
	...
	Model<-list(reacID=reacID,namesSpecies=namesSpecies,interMat=interMat,notMat=notMat)	
	return(Model)
	}
