...
#it returns true if the re is an in/out missing
			...
			if(any(reac2remove)){
				...
				newreacID<-Model$reacID[-reac2remove]
					...
					newModel<-list(reacID=Model$reacID,namesSpecies=newspecies,interMat=newInterMat,notMat=newNotMat)
					}
			newModel<-list(reacID=newreacID,namesSpecies=newspecies,interMat=newInterMat,notMat=newNotMat)
			}
	return(newModel)
	}
