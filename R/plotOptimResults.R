plotOptimResults <-
function(SimResults=SimResults,expResults=expResults,times=times,namesCues=namesCues,namesSignals=namesSignals,valueCues=valueCues){
#Set graphical parameters	
	par(mfrow=c(nr=dim(SimResults[[1]])[1]+1,nc=dim(SimResults[[1]])[2]+1),cex=0.5,pch=20,mar=c(0.5,0.5,0,0),oma=c(3,2,2,2))
...
#Plot 
			if(r == dim(expResults[[1]])[1] && c== 1){
				...
				points(x=xVal,y=yVal,ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n")
				lines(x=xVal,y=yVal,ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n")
				lines(x=xVal,y=yValS,ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n",col="blue",lty=2)
				points(x=xVal,y=yValS,ylim=c(yMin, yMax),xlab=NA,ylab=NA,xaxt="n",yaxt="n",col="blue")
				axis(side=1,at=xVal)
