...
{ 
  ...
  { 
    ...
    if(length(tempIndex)>0) 
    {
      ...
      ODEindexSignals=c(ODEindexSignals,tempIndex);
      CNOindexSignals=c(CNOindexSignals,i);
    }
  }
...
createODEModel<-function(CNOlist,CNOModel)
{
  ...
  odePars=writeCfunction(TTModel);
  
  compileResult=compileAndLoad("myODE");
  
  odePars=addParsInfo(odePars,c("k","tau"));
...
addParsInfo=function(odePars,initType)
{
  LB=odePars$parValues;
  UB=odePars$parValues;
  LB[odePars$index_k]=0.001;
  LB[odePars$index_tau]=0.001;
  LB[odePars$index_n]=1;
  UB[odePars$index_k]=1;
  UB[odePars$index_tau]=10;
  UB[odePars$index_n]=5;
  
  index_opt=c(); 
  if(length(which(initType=='k'))>0)
  {
        index_opt=c(index_opt,odePars$index_k);
  }
  
  if(length(which(initType=='n'))>0)
  {
        index_opt=c(index_opt,odePars$index_n);
  }
  
  if(length(which(initType=='tau'))>0)
  {
        index_opt=c(index_opt,odePars$index_tau);
  }
  odePars$index_opt=index_opt;
  odePars$LB=LB;
  odePars$UB=UB;
...
getObjectiveFunction <-function(x,CNOlist,odePars)
{
  evaluate<-function(x,CNOlist=CNOlist,odePars=odePars)
  { 
    odePars$paramValues[odePars$index_opt]=x;
    indexes=indexCNO2ODE(CNOlist,odePars); 
    simData=simulateData(CNOlist,indexes,odePars);
    times=CNOlist$timeSignals;
    expResults=CNOlist$valueSignals;  
    SimResults=convertTimeSeriesToList(simData,CNOlist);
    mySum=0;
    for(i in 1:length(SimResults))
    {
      mySum=mySum+(SimResults[[i]]-expResults[[i]])^2; 
    }
    mySum=sum(mySum);
...
getObjectiveFunction <-function(CNOlist1,odePars1)
{
  evaluateModel<-function(x,CNOlist=CNOlist1,odePars=odePars1)
  { 
    odePars$paramValues[odePars$index_opt]=x;
    indexes=indexCNO2ODE(CNOlist,odePars); 
    simData=simulateData(CNOlist,indexes,odePars);
...
initPars=function(odePars)
{
  index_opt=odePars$index_opt;
  odePars$paramValues[index_opt]=odePars$LB[index_opt]+((odePars$UB[index_opt]-odePars$LB[index_opt])*runif(length(index_opt)));
  return(odePars);
} 
setwd("C:/Users/davidh/Desktop/distProb/");
