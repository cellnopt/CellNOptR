simulatorT1<-function(CNOlist,Model,SimList,indexList){
	...
	Model$notMat<-as.matrix(Model$notMat)
	Model$reacID<-as.vector(Model$reacID)
	
	SimList$finalCube<-as.matrix(SimList$finalCube)
	SimList$ixNeg<-as.matrix(SimList$ixNeg)
	SimList$ignoreCube<-as.matrix(SimList$ignoreCube)
	SimList$maxIx<-as.vector(SimList$maxIx)
