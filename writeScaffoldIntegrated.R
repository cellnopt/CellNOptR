...
writeScaffoldIntegrated<-function(
  ModelComprExpandedIntegr,
	optimResT1,
	optimResT2,
	ModelOriginal,
	CNOlist, 
  ...
  #if (is.na(optimResT1[1])){
   # optimResT1<-list()
    #optimResT1$bString<-rep(0,length(ModelComprExpandedIntegr$reacID))
  #  optimResT1$bString[ModelComprExpandedIntegr$indexIntegr]<-1
  #  optimResT1$StringsTol<-optimResT1$bString 
#  }
  
	
#get the stuff that I need for the sif file
  sif<-getSifInfo(ModelComprExpanded=ModelComprExpandedIntegr,
...
#get the stuff that I need for the dot file
	dot<-getDotInfo(
		ModelComprExpanded=ModelComprExpandedIntegr,
		ModelOriginal=ModelOriginal,
		CNOlist=CNOlist,
		sifFile=sif$sifFile)
...
###
  indexI<-intersect(ModelComprExpandedIntegr$indexIntegr, which(BStimes==1))
  BStimes[indexI]<-2
