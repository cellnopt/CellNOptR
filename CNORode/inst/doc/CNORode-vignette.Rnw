...
The method used here was first published by Wittmann et al. \cite{wittman} by the name of odefy.
...
Additionally we were specially carerful in tackling the main computanional bottlenecks
by implementing CNORode simulation engine in the C languange using the CVODES library \cite{cvodes}.
...
\emph{CNORode} depends on \emph{CellNOptR} and its dependencies (bioconductor
packages), which can be installed in R. In order to install \emph{CellNOptR}, open a R
session and type:
...
(i.e, none of the R packages are installed on your system). Then, you can install
\emph{CNOROde} similarly:

<<installCNORode, eval=FALSE>>=
source("http://bioconductor.org/biocLite.R")
biocLite("CNORode")
@
...
should still be able to install \emph{CellNOptR} and \emph{CNORode} packages and
...
Additionally, for parameter estimation we recommend the use of eSSR. This algorithm 
algorithm is part of the MEIGOR toolbox. Although to the date, this package is
not available in an oficial repository, it can be downloded from 
\url{http://www.ebi.ac.uk/saezrodriguez/cno/meigo/}.
...
Please note a PKN can also be used as Boolean model which will contain only OR type gates.
...
standard Hill or even not to use any transfer function. To illustrate shape of the equations
associated to each dynamic state and the meaning of each parameter will show the differential
of \textit{Mek}. 
...
to be common, nevertheless you can edit several attributes manually. If you print the the 
...
you want to optimize. The field index\_opt\_pars defines which parameters are meant to be optimized.
...
if we want to simulate only the signals present in the CNOlist object or the all species in the model. Additionally
...
As previously mentioned we included a direct interface for two optimization algorithms that allow parameter estimation
Both of these algorithms have specific parameters that can be tunned each specific problem (please check CNORode manual for detailed information). 
For instance, in order to run the the genetic algorithm for 10 iterations and a population of size we can use the following code:
...
In figure \ref{plotInitFit} we can see a worse solution before optimization the genetic algorithm. This solution
is later improved \ref{plotFinalFit} as seen in the example code. 
...
<<label=plotInit,include=TRUE,fig=TRUE>>=
	simulatedData=plotLBodeFitness(cnolist, model,initial_pars)
@
\caption{The initial solution before optimization.}
...
@
\end{center}
\caption{A solution obtained by optimization with a genetic algorithm.}
