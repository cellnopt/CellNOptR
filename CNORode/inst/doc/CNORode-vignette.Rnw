...
\affil[1]{Instituto de Investigaciones Marinas-CSIC, Vigo, Spain and European Bioinformatics Institute, Saez-Rodriguez group, Cambridge, United Kingdom}
...
The method used here was first published by Wittmann et al. \ref{odefy} by the name of odefy.
For a detailed description of the methodology the user is adressed to \ref{odefy} and for 
a published application example \ref{}.
...
available in R such as eSSR (enhanced Scatter Search Metaheuristic for R) and an R genetic
...
by implementing CNORode simulation engine in the C languange using the CVODES library.

This brief tutorial shows how to use CNORode using as a starting point a Boolean model 
and a dataset consisting on a time-series of several proteins. 
...
Furthermore, for parameter estimation we recommend the use of eSSR. This is 
algorithm is part of the MEIGOR toolbox. Although to the date, this package is
not available in any oficial repository, it can be downloded from 
...
find the set of continous parameters which minimize the squared difference
...
the result of calibrating a \textit{prio knowledge network}(PKN) with \emph{CellNOptR}.
...
The example used here is shipped with the R package. In order to load the data and model you
...
a list with the ODE parameters associated with each dynamic state as described in \cite{}.
...
\[\dot{MEK}=	\Bigg( 1 - \frac{Akt^{n_1}/({k_1}^{n_1} + Akt^{n_1})}{1/({k_1}^{n_1} + 1)}\Bigg) \cdop \Bigg(\frac{Raf^{n_2}/({k_2}^{n_2} + Raf^{n_2})}{1/({k_2}^{n_2} + 1)}\Bigg) \]


<<createContPars, eval=TRUE, results=hide>>=
